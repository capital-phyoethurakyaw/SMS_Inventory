...
namespace SMS.PopUp
{
    public partial class PETC0303I_Import : Form
    {
        string firstColHeader = string.Empty;
        Base_BL baseBL;
        T_NyuukinMeisai_Entity NyuukinMeisai_data;
        CommonFunctions cmf;
        PETC0303I_BL PETC30303I;
        L_Log_Entity LLogEntity;
        string[] arraylist;
        public PETC0303I_Import(string[] arl, L_Log_Entity lle )
        {
            InitializeComponent();
            this.LLogEntity = lle;
            this.arraylist = arl;
        ...
        public bool InsertData(int i)
        {
           ...
           // dgv1.Rows[i].Cells["colImage"].Style.BackColor = Color.FromArgb(255, 255, 192);
            ...
            if (dtImport != null)
            {
                ...
                {
                    if (PETC30303I.CheckExistName1_0303I(NyuukinMeisai_data, LLogEntity, check2kanji))
                    {
                        if (PETC30303I.PETC0303I_Insert(NyuukinMeisai_data, LLogEntity, check2kanji))    // Meisai and tetsuyou
                        {
