...
{
    public partial class frmPETC0302I : SMS_Base
    {
        Login_Info loginInfo;
        PETC0302I_BL PETC0302IBL = new PETC0302I_BL();
        ...
        public frmPETC0302I(Login_Info loginInfo)
            : base(loginInfo)
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        /// 
        private void PETC0302I_Load(object sender, EventArgs e)
        {
            ...
            F12_Text = "出力(F12)";
            F11_Text = "取込(F11)";
            FormName = "手数料仕訳作成(すご楽)";
            dgvDetail.EnabledColumn("colchk");
            dgvPETC0302I.Columns["colNo"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            dgvDetail.Columns["colNo1"].DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter;
            dgvPETC0302I.DisabledColumn("colNo,colNyuukinMotoName1,colKouzaNyuukinDate,col_nyuukingaku");
            lblMode.Visible = false;
            dgvDetail.Gridcolor(Color.Black);
            dgvPETC0302I.Gridcolor(Color.Black);
            txtdocDate.Text = DateTime.Now.ToString("yyyy/MM/dd");
            dgvPETC0302I.EditingControlShowing += datagridview_EditingControlShowing;
            ...
            for (int j = 0; j < 15; j++ )
            {
                ...
                dgvPETC0302I.Columns["colKeihi" + col + "Input"].ValueType = typeof(string);
                dgvPETC0302I.Columns["colKeihi" + col + "Input"].HeaderText = dt.Rows[0]["KeihiTitle" + col].ToString();
            ...
            DisableHeaderFlexible(dgvPETC0302I);
        ...
        public DataTable CommmaSeparatedinOutput(DataTable dt1, string[] cols)
        {
            ...
            {
                if (dtCloned.Columns[i].ColumnName.Contains("借方金額") || dtCloned.Columns[i].ColumnName.Contains("貸方金額"))
                {
        ...
        public void BindlabelBack()
        {
           var f= lbl_total1.BackColor.Name ;
        }
        ...
        public  DataTable  Tesuyou_dt()
        {
            ...
            for (int j = 0; j <  dgvPETC0302I.Rows.Count; j++)
            { 
                dt.Rows.Add();
                dt.Rows[j]["OperatorCD"] = loginInfo.OperatorCode;
                dt.Rows[j]["DetaKBN"] = "1";
                dt.Rows[j]["coltesu_chk"] = Convert.ToBoolean(dgvPETC0302I.Rows[j].Cells["coltesu_chk"].EditedFormattedValue) ? "0" : "1";
                dt.Rows[j]["KeijouDate"] = dgvPETC0302I.Rows[j].Cells["coldocDate"].Value;
            }
        ...
        /// <summary>
        /// dgv to disable header flexible
        /// </summary>
        /// <param name="dg"></param>
        ...
        /// <summary>
        /// Open Folder when click on button
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        ...
        /// <summary>
        /// Delete & Insert data into T_NyuukinMeisai and T_Tesuuryou Table, And Bind data to DataGridViews
        /// </summary>
        private void F11()
        {
                ...
                if (!string.IsNullOrWhiteSpace(txtdocDate.Text))
                {
                     if (DSP_MSG("Q101", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty).Equals("1"))
                     {
                        ...
                        if (datatableCheckColumn(dtCSV))
                        {
                            ...
                            if (PETC0302IBL.T_NyuukinMeisai_T_Tesuuryou_Delete(NyuukinMeisai_data, Tessuryou_data, LLogEntity))
                            {
                                // テーブル転送仕様Ａ ,テーブル転送仕様Ｂ
                                        ...
                                        if ((dtCSV.Select("口座入金日 <> ''").CopyToDataTable().Rows.Count) > 0)
                                        {
                                           ...
                                           //{
                                            NyuukinMeisai_data.xml = baseBL.DataTableToXml(dtNyuukin);
                                               ...
                                               {
                                                   if (PETC0302IBL.PETC0302I_Insert(NyuukinMeisai_data, Tessuryou_data, LLogEntity))
                                                   {
                                                       // 画面転送表01 and 画面転送表02
        ...
        private bool CheckMotoName(T_NyuukinMeisai_Entity NyuukinMeisai)
        {
            if (PETC0302IBL.Check_Motonames(NyuukinMeisai))
            {
        ...
        /// <summary>
        /// Update data from DataGridViews and Export CSV Files
        /// </summary>
        private void F12()
        {
            if (ErrorCheckForGrid())
            {
                if (DSP_MSG("Q203", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty).Equals("1"))
                {
                    ...
                    if (PETC0302IBL.Update_Tesuyou_Nyuukin(Tesuyou_dt(), Nyuukin_dt()))
                    {
                        ...
                        Export_CSV(PETC0302IBL.NyuukinDate_NyuukinMoto_Select(NyuukinMeisai_data.OperatorCD,"1"), PETC0302IBL.PETC0302I_CSVExport1(NyuukinMeisai_data, "PETC0302I_ExportCSV"));
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Check Required Datatable Columns Is Exist
        /// </summary>
        /// <param name="dt"></param>
        /// <returns></returns>
        ...
        /// <summary>
        ///  Bind data to gridview( Bind This After F11)
        /// </summary>
        public void BindGrid(string date)
        {
            ...
            dtShow = PETC0302IBL.PETC0302I_SELECT(Tessuryou_data);
            ...
            dgvPETC0302I.DataSource = dtCloned;
            for (int c = 1; c <= 15; c++)
            {
                decimal result = 0;
                foreach (DataGridViewRow gr in dgvPETC0302I.Rows)
                {
                 ...
                 //   gr.Cells["colKeihi" + val + "Tax"].Value = Calculate_Tax(gr.Cells["colKeihi" + val + "TaxKBN"].Value.ToString(), gr.Cells["colTaxRate"].Value.ToString(), gr.Cells["colKeihi" + val + "input"].Value.ToString().Replace(",", ""));
                    ...
                    if (gr.Index + 1 == dgvPETC0302I.Rows.Count)
                    {
            ...
            dtDetail = PETC0302IBL.PETC0302I_Detail_SELECT(NyuukinMeisai_data);
        ...
        //protected override bool ProcessCmdKey(ref System.Windows.Forms.Message msg, Keys keyData)   // tested as parallel process vs dgv user control
        //{
        //    return false;
        //}

        /// <summary>
        /// 伝票日付 as required field and check for dateformat when chechbox is checked
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        /// 
        public void dgvPETC0302I_CellValidated(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((sender as DataGridView).CurrentCell is DataGridViewTextBoxCell)
            {
                ...
                string[] dn = null;
                if ((Convert.ToBoolean(dgvPETC0302I.Rows[e.RowIndex].Cells["coltesu_chk"].EditedFormattedValue) == true) && (dgvPETC0302I.CurrentCell == dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"]))
                {
                    if (dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value == null)
                    {
                        DSP_MSG("E102", string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                    }
                    else
                    {
                        Date = dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value.ToString();
                        chk_date = checkDate(Date, dgvPETC0302I.CurrentCell);
                        dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value = chk_date;
                        if (chk_date != null)
                        {
                            MotoCD = dgvPETC0302I.Rows[e.RowIndex].Cells["colNyuukinMotoCD"].Value.ToString();
                            KouzaDate = dgvPETC0302I.Rows[e.RowIndex].Cells["colKouzaNyuukinDate"].Value.ToString();
                            string[] moto_kouza = { MotoCD, KouzaDate };
                            CheckALL((sender as DataGridView), "blur", moto_kouza, false, chk_date);
                        }
                    }
                }
                else if ((dgvPETC0302I.CurrentCell == dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"]) && (!(dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value == null)))
                {
                    Date = dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value.ToString();
                    chk_date = checkDate(Date, dgvPETC0302I.CurrentCell);
                    dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].Value =chk_date ;
            ...
            if (((sender as DataGridView).CurrentCell.ColumnIndex >= (sender as DataGridView).CurrentCell.DataGridView.Columns["colKeihi01input"].Index))
            {
                ...
                else  // set value tax and set as separated comma values
                {
                    ...
                    var tax_rate = dgvPETC0302I[(sender as DataGridView).CurrentCell.DataGridView.Columns["coldocDate"].Index, (sender as DataGridView).CurrentCell.RowIndex].Value;
                    ...
                    var kbn = dgvPETC0302I.Rows[(sender as DataGridView).CurrentCell.RowIndex].Cells[kbn_col].Value;
                    ...
                    (sender as DataGridView)[tax_col, (sender as DataGridView).CurrentCell.RowIndex].Value = Calculate_Tax(kbn.ToString(), GetTaxtRate(dgvPETC0302I.Rows[e.RowIndex].Cells["coldocDate"].EditedFormattedValue), input.ToString());
        ...
        public void SetParticularCell(DataGridViewCell dv)
        {
         
            dv.Selected = true;
            dv.DataGridView.BeginEdit(true);
        }
        ...
        public void none_select(DataGridView dgv)   // Clear Selection for all cells  (not used yet)
        {
            for (int w = 0; w < dgv.Rows.Count; w++)
            {
                dgv.Rows[w].Cells[4].Selected = false;
            }
        }

        private void dgvPETC0302I_SelectionChanged(object sender, EventArgs e)
        {
            //if (dgvPETC0302I.CurrentCell.ColumnIndex != 0 && dgvPETC0302I.CurrentCell.ColumnIndex != 3 && dgvPETC0302I.CurrentCell.ColumnIndex != 1)
            //{
            //    if (set_focus)
            //    {
            //        //selected true
            ...
            //    else
            //    {
            //        dgvPETC0302I.ClearSelection();
            //    }
            //}
        }

        //make catch when dataerror 
        public void dgvPETC0302I_DataError(object sender, DataGridViewDataErrorEventArgs anError)
        {
            (sender as DataGridView).EditingControl.ResetText();
            (sender as DataGridView).EditingControl.Text = "0";
            dgvPETC0302I.CellEndEdit  += dgvPETC0302I_CellEndEdit;
            if (!TabSkip)   
            {
        ...
        private bool ErrorCheckForGrid()
        {
            ...
            int Row   ;
            int Column;
             var tupledate = Getlocation(dgvPETC0302I, "");
            ...
            if (Row.ToString() != "-1" && Column.ToString() != "-1")
            {
                ...
                dgvPETC0302I.CurrentCell = dgvPETC0302I[Column, Row];
                return false;
            }
            var tupleminus= Getlocation(dgvPETC0302I,"-");
        ...
        public Tuple<string, string> Getlocation(DataGridView dg, string Check_params)
        {
            var colindex = "-1";
            var rowindex = "-1";
            foreach (DataGridViewRow row in dgvPETC0302I.Rows)
            {
                if (Check_params == "")
                {
        ...
        private void dgvPETC0302I_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            Maintained_CellClick(sender, e);
            
        }

        private void dgvPETC0302I_CellContentDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            Maintained_CellClick(sender, e);
        }
        ...
        protected void Maintained_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if ((sender as DataGridView).CurrentCell is DataGridViewCheckBoxCell)
            {
                ...
                if (e.RowIndex != -1)
                {
                    if ((Convert.ToBoolean(dgvPETC0302I.Rows[e.RowIndex].Cells[e.ColumnIndex].EditedFormattedValue) == true))
                    {
                        MotoCD = dgvPETC0302I.Rows[e.RowIndex].Cells["colNyuukinMotoCD"].Value.ToString();
                        KouzaDate = dgvPETC0302I.Rows[e.RowIndex].Cells["colKouzaNyuukinDate"].Value.ToString();
                        string[] moto_kouza = { MotoCD, KouzaDate };
                ...
                foreach (DataGridViewRow row1 in dgvPETC0302I.Rows)
                {
                    if (row1.Cells["colNyuukinMotoCD"].Value.ToString() == MotoCD && row1.Cells["colKouzaNyuukinDate"].Value.ToString() == KouzaDate)
                    {
                        ...
                        //row1.Cells["col_nyuukingaku"].Value = CommaSeparate(res.ToString());  // by Ptk 2020/01/31 Already in DGV
                    }
                }
            }
        }
        ...
        private void dgvPETC0302I_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)   // Error skiiped when not clicking on header bar
            {
                if (dgvPETC0302I.CurrentCell.ColumnIndex != 0 && dgvPETC0302I.CurrentCell.ColumnIndex != 3 && dgvPETC0302I.CurrentCell.ColumnIndex != 1 && e.RowIndex >= 0)  // must not be these rows
                {
                    dgvPETC0302I.CurrentCell = dgvPETC0302I.Rows[e.RowIndex].Cells[e.ColumnIndex];
                    dgvPETC0302I.CurrentCell.Selected = true;  /// as soon as cursor point set the current cells, making it the cell edit
                    dgvPETC0302I.BeginEdit(true);  // make event for setting flags
                }
            }
        }
        ...
        {
            decimal result = 0;
            var col_num = dgvPETC0302I.Columns[colindex].DataPropertyName.ToString().Replace("Keihi", "").Replace("Input", "").Replace("input", "");
            foreach (DataGridViewRow gr in dgvPETC0302I.Rows)
            {
        ...
        private void dgvPETC0302I_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= (sender as DataGridView).Columns["colKeihi01input"].Index)
            {
                ...
                dgvPETC0302I.NotifyCurrentCellDirty(true);   // put this not occuring win box errors for abnormal changing in cell values
            }
        }

        private void dgvPETC0302I_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            dgcell = (sender as DataGridView).CurrentCell;   // store this value when reselection on error dgv occurred (may not be used)
        }

        /// <summary>
        /// Bind Date From Document Date TextBox to dgvPETC0302I date column
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void txtdocDate_KeyDown(object sender, KeyEventArgs e)  // Check when import and fill each respective cells date fileds
        {
        ...
        private void dgvPETC0302I_KeyDown(object sender, KeyEventArgs e)   // (not need just for tested )
        {
        ...
        private void dgvPETC0302I_CellLeave(object sender, DataGridViewCellEventArgs e)
        {
        ...
        private void btnAllCheckOff_Click(object sender, EventArgs e) //All Check Off
        {
            ...
            CheckALL(dgvPETC0302I, "coltesu_chk",null,false,null);
        ...
        private void MaintainCellClick_detail(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if ((sender as DataGridView).CurrentCell is DataGridViewCheckBoxCell)
            {
                ...
                if ((Convert.ToBoolean(dgvDetail.Rows[e.RowIndex].Cells[e.ColumnIndex].EditedFormattedValue) == true))
                {
                    ...
                    string[] moto_kouza = { MotoCD, KouzaDate };
                    CheckALL(dgvPETC0302I, "blur", moto_kouza, true);
                ...
                else
                {
                   // this.dgvPETC0302I.Refresh();
                    ...
                    if (!flg)
                    {
                        ...
                        KouzaDate = dgvDetail.Rows[e.RowIndex].Cells["colKouzaNyuukinDate1"].Value.ToString();
                        string[] moto_kouza = { MotoCD, KouzaDate };
                        CheckALL(dgvPETC0302I, "blur", moto_kouza, false);
                    }
        ...
        private void dgvPETC0302I_CurrentCellDirtyStateChanged(object sender, EventArgs e)
        {
        ...
        private void dgvPETC0302I_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
